...
public class TerceirizadosDAO extends GenericDAO<Terceirizados> {
	...
	public void salvarFuncionarioTerceirizado(Unidade unidade, FuncionarioTerceirizado funcionario,
			Terceirizados terceirizado, Usuario usuarioLogado) {
	...
	@SuppressWarnings("unchecked")
	public List<Terceirizados> listarPorUnidadeCodigo(UnidadeFunc processo, Setor setor) {
		Session sessao = HibernateUtil.getFabricaDeSessoes().openSession();
		try {
			Criteria consulta = sessao.createCriteria(Terceirizados.class);
			consulta.add(Restrictions.eq("unidadeFunc", processo));
